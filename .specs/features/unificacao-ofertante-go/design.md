...
| Documento duplicado (CPF ou CNPJ, `documento` já pertence a outro usuário) | ~~Unicidade de PK → exceção do Prisma não tratada explicitamente, convertida por `comTratamentoDeErro` (mesmo padrão de CPF duplicado hoje)~~ **Correção pós-Verifier (ranked gap #2, iteração 1)**: não havia, de fato, nenhum precedente de "erro claro" pré-existente para CPF duplicado a reaproveitar — a linha original descrevia uma suposição, não um padrão real. AC4 (P2) exige "erro claro de duplicidade", não um 500 genérico; `POST /api/usuarios` agora faz `findUnique` por `documento` antes do `create` e responde 409 explicitamente, para GO e para os demais tipos | 409 com mensagem clara ("CNPJ já cadastrado para outro Gestor Ofertante" / "Documento já cadastrado") — o 500 genérico de `comTratamentoDeErro` continua existindo só como rede de segurança para a corrida residual entre a checagem e o `create` |
