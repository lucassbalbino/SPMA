...
**Correção (T19): diagnóstico anterior estava errado.** O parágrafo abaixo (mantido riscado por transparência, não apagado) atribuiu a falha de `deleteUsuarios` a uma suposta violação da FK auto-referenciada `Usuario.cdOfertante`. Verificado empiricamente ao chegar em T19 (script isolado: cria GO+VO com `cdOfertante` apontando um pro outro, `deleteMany` dos dois numa chamada só): a migration `20260912170000_unificar_ofertante_go` já define essa FK como `ON DELETE SET NULL` (`prisma/migrations/.../migration.sql:84`) - apagar o GO nunca falha, só zera o `cdOfertante` do VO. A causa real da falha original era outra, e já estava corretamente resolvida no mesmo commit: a Verba criada pelo teste "AM cria GO" (T15) apontava para `CNPJ_NOVO_GO`, que não estava na lista de `deleteVerbasPorOfertante`/`deletePreCursosPorOfertante` do `afterAll` - `Verba.cdOfertante`/`PreCurso.cdOfertante` SÃO `ON DELETE RESTRICT` (mesmo arquivo de migration, linhas 92-93), e essa é a FK que de fato bloqueava. A mudança em duas passadas no `deleteUsuarios` (mantida no código - inofensiva, só uma query a mais) não era necessária para este caso, mas segue sendo uma proteção razoável caso outra FK futura em `Usuario` vire `RESTRICT`. ~~**Achado bloqueante corrigido (helper de fixture)**: `deleteUsuarios` em `scripts/e2e-fixture.ts` fazia um único `deleteMany` - inofensivo enquanto `Usuario.cdOfertante` apontava para uma tabela `Ofertante` à parte, mas agora que é auto-referência (`Usuario.documento`, VO→GO) um `deleteMany` sem ordem garantida pode tentar apagar o GO antes do VO que o referencia na mesma instrução, violando a FK. Corrigido em duas passadas (primeiro quem tem `cdOfertante` preenchido, depois o resto) - helper genérico, beneficia qualquer e2e futuro que crie VO+GO juntos, não só este arquivo.~~
