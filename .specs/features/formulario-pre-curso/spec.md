...
| Estratégia de armazenamento das respostas do questionário (`Json?` vs coluna por pergunta) | ~~JSON validado por Zod~~ → **uma linha por resposta em `TB_Resposta_Pre_Curso`** (AD-041, 2026-09-11, rescinde a AD-034 que esta feature havia formalizado) | O Zod continua sendo a autoridade de forma (AD-004) e o domínio continua vendo `{ chave: valor }`: a troca ficou contida no repositório, sem alterar nenhum requisito nem asserção desta feature | y |
...
| Quem pode criar pré-curso | GO (Gestor Ofertante) do próprio Ofertante, ou AM (qualquer Ofertante) | Seção 4 do doc original: "Preenchido pelo Gestor Ofertante"; regra ampliada pelo product owner em AD-040 (2026-09) para dar ao AM a mesma autoridade nacional que já tem em Usuário/Verba/matrícula (AD-012) — supersede esta linha, ver `.specs/features/criacao-curso-por-am/spec.md` | y |
