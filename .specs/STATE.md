...
- **Próxima feature:** `cadastro-ofertante-verba` (README, ordem sugerida item 3). **Sem spec.md ainda** - ao contrário das duas primeiras features, esta precisa da fase Specify do zero, extraindo requisitos de `SPMA_Especificacao_Cliente_v2.md` (AD-014 a AD-016 no `## Decisions` acima já cobrem as decisões de arquitetura de Ofertante/Verba a respeitar).
- **Pendências abertas:** nenhuma de domínio, exceto os indicadores do dashboard (feature adiada por decisão AD-024) e os dois fechamentos e2e diferidos de `seguranca-transversal` citados acima (CA-SEC-14, CA-SEC-17) a resolver quando as features correspondentes chegarem.
