...
- **Fase atual:** `auth-e-usuarios` — **30/30 tarefas (T1-T30) implementadas e commitadas**. Verifier independente (sub-agente fresco, author≠verifier) despachado e em execução; a feature só conta como concluída quando `.specs/features/auth-e-usuarios/validation.md` existir com veredito PASS e `validate_state.py` confirmar. Ao retomar: primeiro checar se o Verifier já reportou; se PASS, atualizar `tasks.md` Status para `Done` e este Handoff; se FAIL, rotear os gaps como fix tasks (máx. 3 ciclos fix→reverify antes de escalar).
- **Testes no HEAD:** 135 (85 unit / 14 integration / 36 e2e), todos verdes. `docker compose ps` deve mostrar `spma-mysql` healthy antes de rodar integration/e2e.
...
- **Arquivos da feature:** `.specs/features/auth-e-usuarios/{spec.md,design.md,tasks.md}` (+ `validation.md` quando o Verifier terminar).
